...
public utilisateur utilisateur; 
    ...
    @FXML
    private void fdefiequipe(ActionEvent event) {
        try {
            FXMLLoader loader=new  FXMLLoader(getClass().getResource("defiequipe.fxml"));
            Parent root=loader.load();
           DefiequipeController pdc = loader.getController();
          pdc.setEquipe(equipe);
          pdc.setUtilisateur(utilisateur);
            logo.getScene().setRoot(root);
        } catch (IOException ex) {
    ...
    @FXML
    private void fcapitain(MouseEvent event) throws SQLException {
         ObservableList<utilisateur> s=tablemember.getSelectionModel().getSelectedItems();
         if(!s.isEmpty()){
           ServiceEquipe se=new ServiceEquipe();
           
           equipe.setCapitain(s.get(0).getId());
           se.update(equipe);
           try {
    ...
    @FXML
    private void faccepterdefi(ActionEvent event) throws SQLException {
         try {
            FXMLLoader loader=new  FXMLLoader(getClass().getResource("accepterdefi.fxml"));
            Parent root=loader.load();
           AccepterdefiController pdc = loader.getController();
          pdc.setEquipe(equipe);
          pdc.setUtilisateur(utilisateur);
            logo.getScene().setRoot(root);
        } catch (IOException ex) {

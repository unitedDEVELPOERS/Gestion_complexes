...
//        nouveauTerrain();
    ...
    private void btnEnregistrerUpdate(ActionEvent event) {
        ...
        {
            Terrain tt = new Terrain(terrain.getId(),tfNom.getText(), taDesc.getText(), gouv, tfImage.getText(), Double.parseDouble(tfPrix.getText()),c,ca,timeOuvert.getValue(), timeFerm.getValue());
              terrain.setDesignation(tfNom.getText());
              terrain.setDescription(taDesc.getText());
              terrain.setAdresse(gouv);
...
//                alertMail();
    ...
    private void btnEnregistrerAdd(ActionEvent event) {
        if(tfNomAdd.getText().length()>0 && taDescAdd.getText().length()>0 && cbCategorieAdd.getSelectionModel().getSelectedIndex()!=-1
            ...
            && tfImageAdd.getText().length()>0 && tfPrixAdd.getText().length()>0 && isNumeric(tfPrixAdd.getText()) && timeOuvertAdd.getValue()!=null && timeFermAdd.getValue()!=null)              
        {
            Terrain terrain = new Terrain(tfNomAdd.getText(), taDescAdd.getText(), cbGouvernoratAdd.getSelectionModel().getSelectedItem().toString(), tfImageAdd.getText(), Double.parseDouble(tfPrixAdd.getText()),c,(Categorie)cbCategorieAdd.getSelectionModel().getSelectedItem(),timeOuvertAdd.getValue(), timeFermAdd.getValue());
